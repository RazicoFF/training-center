...
fun ScheduleScreen(viewModel: ScheduleViewModel, padding: PaddingValues) {
    ...
    Box(modifier = Modifier.fillMaxSize().padding(padding)) {
        when (val s = state) {
            ...
            is ScheduleUiState.Error -> Text(s.error.asString(), modifier = Modifier.align(Alignment.Center))
...
private fun ScheduleRow(item: ScheduleItemDto) {
    Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp)) {
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            Text(stringResource(R.string.schedule_when, item.lessonDate, item.startTime, item.endTime))
